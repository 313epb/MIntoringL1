...
namespace MvcMusicStore.Controllers
{
	...
	public class AccountController : Controller
	{
		...
		public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
		{
			...
			{
				Logger.Info("Login model is valid.");
				...
				if (user != null)
				{
					...
					Logger.Info("Redirecting to local.");

					return RedirectToLocal(returnUrl);
				}

				Logger.Info("User {0} is not found.", model.UserName);
		...
		public async Task<ActionResult> Register(RegisterViewModel model)
		{
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					...
					Logger.Info("{0} has registered successfully.", user.UserName);
					
					return RedirectToAction("Index", "Home");
				}

				Logger.Info("There were errors while registering of user {0}", user.UserName);
		...
		public ActionResult LogOff()
		{
			...
			using (var logOffCounter = PerformanceCounterFactory.CreateLogOffCounter())
			{
		...
		private async Task SignInAsync(ApplicationUser user, bool isPersistent)
		{
			...
			using (var logInCounter = PerformanceCounterFactory.CreateLogInCounter())
			{
